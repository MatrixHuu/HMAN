...
class reasoning(nn.Module):
     ...
     def forward(self, frame_feats, i3d_feats, object_feats, phr_feats, last_hidden, phr_masks):
            ...
            semantic_group_feats = torch.cat([semantic_frame_feats, semantic_i3d_feats, phr_feats], dim=2)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        nn.init.xavier_normal_(self.guidance_fc.weight)

        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.word_size*2+opt.feature_size*2+opt.hidden_size, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # language RC lstm
        self.lang_RC_lstm = nn.LSTMCell(opt.word_size*2+opt.feature_size*2+opt.hidden_size, opt.hidden_size)
        self.lstm_RC_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.word_size+opt.feature_size*2+opt.hidden_size*3, self.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        lang_RC_lstm_h, lang_RC_lstm_c = self.get_rnn_init_hidden(batch_size, 512)
        ...
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                # context guidance
                guidance = self.guidance_fc(torch.cat([lang_RC_lstm_h, lang_lstm_h, embedded], dim=1))

                output, lang_lstm_h, lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c = self.decode(frame_feats, i3d_feats, object_feats, guidance,
                                                                                              lang_lstm_h, lang_lstm_c, lang_RC_lstm_h,
                                                                                              lang_RC_lstm_c, embedded, phr_feats, phr_masks)
    ...
    def decode(self, frame_feats, i3d_feats, object_feats, guidance, lang_lstm_h, lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c, embedded, phr_feats, phr_masks):

        feats = self.reasoning(frame_feats, i3d_feats, object_feats, phr_feats, guidance, phr_masks)

        # language lstm decoder
        decoder_input = torch.cat([lang_RC_lstm_h, feats, embedded], dim=1)
        ...
        reconsiderator_input = torch.cat([lstm_h, feats, embedded], dim=1)
        ...
        return word_logits, lstm_h, lstm_c, lstm_RC_h, lstm_RC_c
    ...
    def beam_search(self, batch_size, vocab_size, frame_feats, i3d_feats, object_feats, width):
            ...
            for t in range(0, self.max_words):
                ...
                assert len(input_list) == len(hidden_list) == len(RC_hidden_list) == len(cum_prob_list)
                for i, (input, hidden, RC_hidden, cum_prob) in enumerate(zip(input_list, hidden_list, RC_hidden_list, cum_prob_list)):
                    ...
                    # context guidance
                    guidance = self.guidance_fc(torch.cat([RC_hidden[0], hidden[0], embedded], dim=1))

                    output, lang_lstm_h, lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c = self.decode(frame_feats, i3d_feats, object_feats, guidance,
                                                                                                  hidden[0], hidden[1], RC_hidden[0], RC_hidden[1],
                    ...
                    beam_RC_hidden_list[0].append(lang_RC_lstm_h)
                    beam_RC_hidden_list[1].append(lang_RC_lstm_c)
                ...
                for i, (beam_index, output_index) in enumerate(zip(topk_beam_index, topk_output_index)):
                    for k, (bi, oi) in enumerate(zip(beam_index, output_index)):
                        topk_RC_hidden_list[0][k].append(beam_RC_hidden_list[0][bi][i, :])
                        topk_RC_hidden_list[1][k].append(beam_RC_hidden_list[1][bi][i, :])
                ...
                hidden_list = (
                    ...
                    [torch.stack(topk_hidden, dim=0) for topk_hidden in topk_hidden_list[1]])

                RC_hidden_list = (
                    [torch.stack(topk_hidden, dim=0) for topk_hidden in topk_RC_hidden_list[0]],
                    [torch.stack(topk_hidden, dim=0) for topk_hidden in topk_RC_hidden_list[1]])
                # 2 * width * ( 1, 100, 512 )
                hidden_list = [(hidden, context) for hidden, context in zip(*hidden_list)]

                RC_hidden_list = [(RC_hidden, context) for RC_hidden, context in zip(*RC_hidden_list)]
