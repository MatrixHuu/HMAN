...
class LOCATE(nn.Module):
    def __init__(self, opt):
        ...
        self.temp_attn = SoftAttention(feat_size, opt.hidden_size, opt.att_size)
...
class RELATE(nn.Module):
    def __init__(self, opt):
        ...
        self.spatial_attn = SoftAttention(region_feat_size, opt.hidden_size, opt.att_size)

        # temporal soft attention module
        feat_size = region_feat_size + opt.hidden_size * 2
        self.relation_attn = SoftAttention(2*feat_size, opt.hidden_size, opt.att_size)
...
class FUNC(nn.Module):
    def __init__(self, opt):
        ...
        self.cell_attn = SoftAttention(opt.hidden_size * 2, opt.hidden_size, opt.att_size)
    ...
    def __init__(self, opt):
        ...
        if opt.use_loc and opt.use_rel and opt.use_func:
            if opt.attention == 'soft':
                self.module_attn = SoftAttention(opt.hidden_size, opt.hidden_size, opt.att_size)
            elif opt.attention == 'gumbel':
                self.module_attn = GumbelAttention(opt.hidden_size, opt.hidden_size, opt.att_size)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # context guidance fc
        guidance_insize = opt.hidden_size * 2 + opt.word_size
        self.guidance_fc = nn.Linear(guidance_insize, opt.att_size)

        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.hidden_size * 2 + opt.word_size, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # language RC lstm
        self.lang_RC_lstm = nn.LSTMCell(opt.hidden_size * 2 + opt.word_size, opt.hidden_size)
        self.lstm_RC_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.hidden_size * 3 + opt.att_size, self.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        lang_lstm_h, lang_lstm_c = self._init_lstm_state(frame_feats)
        lang_RC_lstm_h, lang_RC_lstm_c = self._init_lstm_state(frame_feats)
        # att_lstm_h, att_lstm_c = self._init_lstm_state(global_feat)

        # add a '<start>' sign
        start_id = self.vocab('<start>')
        start_id = frame_feats.data.new(frame_feats.size(0)).long().fill_(start_id)
        word = self.word_embed(start_id)
        word = self.word_drop(word)  # b*w

        # initialize context_guidance
        guidance = self.guidance_fc(torch.cat([lang_RC_lstm_h, lang_lstm_h, word], dim=1))

        # training stage
        outputs = []
        previous_cells = []
        lang_cell = torch.cat([lang_lstm_c, lang_RC_lstm_c], dim=-1)
        previous_cells.append(lang_cell)
        module_weights = []
        if not infer or self.beam_size == 1:
            for i in range(self.max_words):
                ...
                word_logits, module_weight, lang_lstm_h, lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c = self.decode(frame_feats, i3d_feats, object_feats,
                                                                                                                   previous_cells, guidance, lang_lstm_h,
                                                                                                                   lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c,
                                                                                                                   word)
            ...
            start_state = {'guidance': guidance, 'lang_lstm_h': lang_lstm_h, 'lang_lstm_c': lang_lstm_c,
                           'lang_RC_lstm_h': lang_RC_lstm_h, 'lang_RC_lstm_c': lang_RC_lstm_c,
                           'frame_feats': frame_feats, 'i3d_feats': i3d_feats, 'object_feats': object_feats,
    ...
    def beam_step(self, last_predictions, current_state):
        ...
        for i in range(num):
            ...
            guidance = current_state['guidance'][:, i, :]
            lang_lstm_h = current_state['lang_lstm_h'][:, i, :]
            lang_lstm_c = current_state['lang_lstm_c'][:, i, :]
            lang_RC_lstm_h = current_state['lang_RC_lstm_h'][:, i, :]
            lang_RC_lstm_c = current_state['lang_RC_lstm_c'][:, i, :]
            ...
            # language lstm decoder + RC
            word_logits, module_weight, lang_lstm_h, lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c = self.decode(frame_feats, i3d_feats, object_feats,
                                                                                                               previous_cells, guidance, lang_lstm_h,
                                                                                                               lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c,
                                                                                                               word)
            lang_cell = torch.cat([lang_lstm_c, lang_RC_lstm_c], dim=-1)
            previous_cells = torch.cat([previous_cells, lang_cell.unsqueeze(1)], dim=1)
            ...
            new_state['guidance'].append(guidance)
            new_state['lang_lstm_h'].append(lang_lstm_h)
            new_state['lang_lstm_c'].append(lang_lstm_c)
            new_state['lang_RC_lstm_h'].append(lang_RC_lstm_h)
            new_state['lang_RC_lstm_c'].append(lang_RC_lstm_c)
    ...
    def decode(self, frame_feats, i3d_feats, object_feats, previous_cells, guidance, lang_lstm_h, lang_lstm_c, lang_RC_lstm_h, lang_RC_lstm_c, word):
        ...
        loc_feats = self.loc(frame_feats, object_feats, guidance) if self.use_loc else None
        rel_feats = self.rel(i3d_feats, object_feats, guidance) if self.use_rel else None
        func_feats = self.func(previous_cells, guidance) if self.use_func else None
        feats, module_weight = self.module_selection(loc_feats, rel_feats, func_feats, guidance)

        # language lstm decoder
        decoder_input = torch.cat([lang_RC_lstm_h, feats, word], dim=1)
        lstm_h, lstm_c = self.lang_lstm(decoder_input, (lang_lstm_h, lang_lstm_c))
        lstm_h = self.lstm_drop(lstm_h)

        # language lstm reconsiderator
        reconsiderator_input = torch.cat([lstm_h, feats, word], dim=1)
        lstm_RC_h, lstm_RC_c = self.lang_RC_lstm(reconsiderator_input, (lang_RC_lstm_h, lang_RC_lstm_c))
        lstm_RC_h = self.lstm_RC_drop(lstm_RC_h)
        reconsiderator_output = torch.tanh(self.out_fc(torch.cat([feats, guidance, lstm_RC_h, lstm_h], dim=1)))
        # decoder_output = torch.tanh(self.out_fc(torch.cat([lstm_h, decoder_input], dim=1)))
        word_logits = self.word_restore(reconsiderator_output)  # b*v
        return word_logits, module_weight, lstm_h, lstm_c, lstm_RC_h, lstm_RC_c
