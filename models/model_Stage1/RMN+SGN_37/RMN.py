...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.hidden_size * 3, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.hidden_size * 4, self.hidden_size)
        ...
        self.vis_embedded = nn.Linear(opt.hidden_size * 2, self.word_size)

        self.decoder = Feat_Decoder(
            vis_feat_size=opt.word_size,
            feat_len=opt.max_frames,
            embedding_size=opt.word_size,
            sem_align_hidden_size=opt.hidden_size,
            sem_attn_hidden_size=opt.hidden_size,
            hidden_size=opt.hidden_size)

        self.featembedded = nn.Linear(4*opt.word_size, opt.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        # vis_encoder
        app_feat = self.vis_embedded(frame_feats)
        mot_feat = self.vis_embedded(i3d_feats)
        vis_feat = torch.cat([app_feat,mot_feat], dim=2)
        ...
        if not infer or self.beam_size == 1:
            for i in range(self.max_words):
                ...
                if i >= 1:
                    ...
                    A_mask = torch.eye(i , i).cuda().type(torch.bool)
                    ...
                    phr_masks = None
                feat, (sem_align_weights, _), (sem_align_logits, _) = self.decoder(
                    word, att_lstm_h, vis_feat, phr_feats, phr_masks)
                ...
                word_logits, module_weight, lang_lstm_h, lang_lstm_c = self.decode(feat, frame_feats, i3d_feats, object_feats,
    ...
    def beam_step(self, last_predictions, current_state):
        ...
        for i in range(num):
            ...
            vis_feat = current_state['vis_feat'][:, i, :]
            ...
            new_state['vis_feat'].append(vis_feat)
    ...
    def decode(self, sele_feat, frame_feats, i3d_feats, object_feats, previous_cells, att_lstm_h, lang_lstm_h, lang_lstm_c):
        ...
        sele_feat = self.featembedded(sele_feat)
        # language lstm decoder
        decoder_input = torch.cat([feats, sele_feat, att_lstm_h], dim=1)
