...
import numpy as np
from model.evaluate import evaluate, convert_data_to_coco_scorer_format
...
def main(opt):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, (frames, regions, spatials, captions, pos_tags, cap_lens, video_ids) in enumerate(train_loader, start=1):
            ...
            optimizer.zero_grad()
            outputs, module_weights, mask_index = net(frames, regions, spatials, targets, epsilon)
            tokens = outputs
            bsz = len(captions)
            ...
            # loss_count /= 10 if bsz == opt.train_batch_size else i % 10
                print('Epoch [%d/%d], Step [%d/%d], Loss: %.4f, Perplexity: %5.4f' %
                      (epoch, opt.max_epoch, i, total_step, loss_count,
                       np.exp(loss_count)))
                mask_res_per_frame = mask_index.sum(dim=2).type(torch.float)
                mask_res_per_video = mask_res_per_frame.sum(dim=1)
                mask_res_per_batch = mask_res_per_video.mean(dim=0)/opt.max_frames
                print('mask_per_video', mask_res_per_video)
                print('mask_mean_frame', mask_res_per_batch)
                loss_count = 0
