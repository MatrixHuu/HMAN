...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        self.SelAtten = SelfAttention(opt.region_feature_size, opt.region_feature_size, opt.region_feature_size, opt.dropout, False)
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats):
        ...
        bsz, max_frames, num_boxes, fsize = region_feats.size()
...
class LOCATE(nn.Module):
    def __init__(self, opt):
        ...
        # self attention
        input_size = opt.region_projected_size
        self.self_attention = SelfAttention(input_size, input_size, input_size, opt.dropout, True, True)
    ...
    def forward(self, frame_feats, object_feats, hidden_state, mask):
        ...
        object_feats_att, _ = self.self_attention(object_feats_att)
...
class RELATE(nn.Module):
    ...
    def forward(self, i3d_feats, object_feats, hidden_state, mask):
...
class Decoder(nn.Module):
    ...
    def decode(self, frame_feats, i3d_feats, object_feats, previous_cells, att_lstm_h, lang_lstm_h, lang_lstm_c, mask):
        ...
        loc_feats = self.loc(frame_feats, object_feats, att_lstm_h, mask) if self.use_loc else None
        rel_feats = self.rel(i3d_feats, object_feats, att_lstm_h, mask) if self.use_rel else None
...
class CapModel(nn.Module):
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats, captions, teacher_forcing_ratio=1.0):
        ...
        outputs, module_weights = self.decoder(frame_feats, i3d_feats, object_feats, captions, mask_index,
                                               teacher_forcing_ratio)
        return outputs, module_weights, mask_index