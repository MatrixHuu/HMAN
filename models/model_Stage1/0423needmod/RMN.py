...
class Decoder(nn.Module):
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        if not infer or self.beam_size == 1:
            ...
            batch_size = dim2_feats.size(0)
            outputs = self.beam_search(batch_size, self.vocab_size, frame_feats, i3d_feats, object_feats, self.beam_size)
    ...
    def beam_search(self, batch_size, vocab_size, frame_feats, i3d_feats, object_feats, width):
        ...
        for t in range(self.max_words):
            ...
            for i, (input, hidden, cum_prob) in enumerate(zip(input_list, hidden_list, cum_prob_list)):
                ...
                phr_masks = self.phr_mask(t, phr_attns, self.PS_threshold)
            ...
            hidden_list = (
                ...
                [torch.stack(topk_hidden, dim=0) for topk_hidden in topk_hidden_list[1]])  # 2 * width * ( 1, 100, 512 )
