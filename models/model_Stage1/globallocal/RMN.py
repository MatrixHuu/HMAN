...
class reasoning(nn.Module):
     ...
     def forward(self, vis_feats, object_feats, phr_feats, last_hidden, phr_masks):
            """
            ...
            global_feats = self.cnnembedded(vis_feats)
            bsz, max_frames, num_boxes, fsize = object_feats.size()
            all_obj = object_feats.view(bsz, max_frames * num_boxes, -1)
            semantic_obj_feats, semantic_obj_weights, semantic_obj_logits = self.semantic_alignment(
                phr_feats=phr_feats,
            ...
            semantic_global_feats, _, _ = self.semantic_alignment(
                phr_feats=phr_feats,
                vis_feats=global_feats)

            semantic_group_feats_1 = torch.cat([semantic_global_feats,  phr_feats], dim=2)    #1324
            semantic_group_feats_2 = torch.cat([semantic_obj_feats, phr_feats], dim=2)       #1324


            feat, semantic_attn_weights, semantic_attn_logits = self.semantic_attention1(
                query=last_hidden,
                keys=semantic_group_feats_1,
                values=semantic_group_feats_1,
                masks=phr_masks)


            feato, semantic_attn_weights, semantic_attn_logits = self.semantic_attention2(
                query=last_hidden,
                keys=semantic_group_feats_2,
                values=semantic_group_feats_2,
                masks=phr_masks)


            allfeat = torch.cat([feat, feato], dim=1)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.word_size*3+opt.feature_size*2, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.word_size*3+opt.feature_size*2+opt.hidden_size, self.hidden_size)
    ...
    def forward(self, vis_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                output, lang_lstm_h, lang_lstm_c = self.decode(vis_feats, object_feats, lang_lstm_h, lang_lstm_c,
            ...
            outputs = self.beam_search(batch_size, self.vocab_size, vis_feats, object_feats,
                                       self.beam_size)
        return outputs

    def decode(self, vis_feats, object_feats, lang_lstm_h, lang_lstm_c, embedded, phr_feats, phr_masks):

        feats = self.reasoning(vis_feats, object_feats, phr_feats, lang_lstm_h, phr_masks)
    ...
    def beam_search(self, batch_size, vocab_size, vis_feats, object_feats, width):
            ...
            for t in range(0, self.max_words):
                ...
                for i, (input, hidden, cum_prob) in enumerate(zip(input_list, hidden_list, cum_prob_list)):
                    ...
                    output, lang_lstm_h, lang_lstm_c = self.decode(vis_feats, object_feats,
                                                                   hidden[0], hidden[1],
                                                                   embedded, phr_feats, phr_masks)
...
class CapModel(nn.Module):
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats, captions, teacher_forcing_ratio=1.0):
        vis_feats, object_feats = self.encoder(cnn_feats, region_feats, spatial_feats)
        outputs = self.decoder(vis_feats, object_feats, captions, teacher_forcing_ratio)
        return outputs