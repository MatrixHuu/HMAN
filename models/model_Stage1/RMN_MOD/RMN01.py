...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.hidden_size * 4, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(2560, self.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        if not infer or self.beam_size == 1:
            for i in range(self.max_words):
                ...
                word_logits,  lang_lstm_h, lang_lstm_c = self.decode(global_frame_feat, global_i3d_feat, object_feats,
    ...
    def decode(self, global_frame_feat, global_i3d_feat, object_feats, previous_cells, att_lstm_h, lang_lstm_h, lang_lstm_c):
        ...
        # language lstm decoder
        decoder_input = torch.cat([global_frame_feat, global_i3d_feat], dim=1)
