...
class SelfAttention(nn.Module):
    # (312, 36, 1000) (312, 512)
    def __init__(self, feat_size=1000, att_size=512):
        super(SelfAttention, self).__init__()
        self.feat_size = feat_size
        # self.pos_size = pos_size
        # self.wp = nn.Linear(pos_size, att_size)     # 5->512
        self.wv = nn.Linear(feat_size, att_size)     # 1000->512
        self.wq = nn.Linear(feat_size, att_size)
        self.wk = nn.Linear(feat_size, att_size)
        self.em = nn.Linear(36 * att_size,feat_size)
        self.dropout = nn.Dropout(0.5)

    def forward(self, feats, pos):
        k = self.wk(feats)
        q = self.wq(feats)
        v = self.wv(feats) # (312, 36, 1000) => (312, 36, 512)
        # p = self.wp(pos.view(312, 36, 5))  # (312, 36, 5) => (312, 36, 512)
        # t_k = k + p
        # t_q = q + p
        # t_v = v + p
        d_k = q.size(-1)
        ...
        atten = self.em(torch.matmul(p_attn, v).view(312, -1))
        return atten, p_attn
'''
...
class LOCATE(nn.Module):
    def __init__(self, opt):
        ...
        # self.spatial_attn = SelfAttention(opt.region_projected_size, 5, opt.hidden_size)
        # GSR
        self.img_rnn = nn.GRU(opt.region_projected_size, opt.region_projected_size, 1, batch_first=True)

        # GCN reasoning
        self.Rs_GCN_1 = Rs_GCN(in_channels=opt.region_projected_size, inter_channels=opt.region_projected_size)
        self.Rs_GCN_2 = Rs_GCN(in_channels=opt.region_projected_size, inter_channels=opt.region_projected_size)
        self.Rs_GCN_3 = Rs_GCN(in_channels=opt.region_projected_size, inter_channels=opt.region_projected_size)
        self.Rs_GCN_4 = Rs_GCN(in_channels=opt.region_projected_size, inter_channels=opt.region_projected_size)
    ...
    def forward(self, frame_feats, object_feats, hidden_state):
        ...
        # GCN reasoning
        # -> B,D,N
        GCN_img_emd = feats.permute(0, 2, 1)
        ...
        GCN_img_emd = self.l2norm(GCN_img_emd)

        rnn_img, rnn_hidden_state = self.img_rnn(GCN_img_emd)

        # features = torch.mean(rnn_img,dim=1)
        features = rnn_hidden_state[0]

        features = self.l2norm(features)

        object_feats_att = features.reshape(bsz, max_frames, fsize)
...
class RELATE(nn.Module):
    def __init__(self, opt):
        ...
        # GSR
        self.img_rnn = nn.GRU(region_feat_size, region_feat_size, 1, batch_first=True)

        # GCN reasoning
        self.Rs_GCN_1 = Rs_GCN(in_channels=region_feat_size, inter_channels=region_feat_size)
        self.Rs_GCN_2 = Rs_GCN(in_channels=region_feat_size, inter_channels=region_feat_size)
        self.Rs_GCN_3 = Rs_GCN(in_channels=region_feat_size, inter_channels=region_feat_size)
        self.Rs_GCN_4 = Rs_GCN(in_channels=region_feat_size, inter_channels=region_feat_size)
...
class ModuleSelection(nn.Module):
    def __init__(self, opt):
        ...
        if opt.use_rel:
            ...
            rel_feat_size = opt.region_projected_size + 2 * opt.hidden_size
        ...
        if not infer or self.beam_size == 1:
            for i in range(self.max_words):
                ...
                word_logits, module_weight, lang_lstm_h, lang_lstm_c = self.decode(frame_feats, i3d_feats, object_feats,
                                                                    previous_cells, att_lstm_h, lang_lstm_h, lang_lstm_c)
