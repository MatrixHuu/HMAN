...
import numpy as np
from March_Model import evaluate, convert_data_to_coco_scorer_format
...
def main(opt):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, (frames, regions, spatials, captions, pos_tags, cap_lens, video_ids) in enumerate(train_loader, start=1):
            ...
            regions = regions.to(DEVICE) # bs 26 36 2048 --> bs 26 18 2048

            regions, index = mask(regions)  #bs  26  18  2048

            spatials = spatials.to(DEVICE) # bs 26 36 5 --> bs 26 18 5

            spatials = obj_mapping(spatials, index)
...
def obj_mapping(A, index):

    bsz, _, _, fsize = A.shape

    dim0, dim1, dim2 = index.shape
