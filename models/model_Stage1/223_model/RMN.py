...
class LOCATE(nn.Module):
    def __init__(self, opt):
        ...
        self.self_attention = SelfAttention(input_size, input_size, input_size, opt.dropout, True)
        self.layernorm_sa = nn.LayerNorm(input_size)
    ...
    def forward(self, frame_feats, object_feats, hidden_state):
        ...
        object_feats_att = self.self_attention(object_feats_att)
        object_feats_att = self.layernorm_sa(object_feats_att)
...
class RELATE(nn.Module):
    ...
    def forward(self, i3d_feats, object_feats, hidden_state):
        ...
        hidden = hidden_state.repeat(1, max_frames).reshape(bsz * max_frames, -1)
        object_feats_att, _ = self.spatial_attn(feats, hidden)
        object_feats_att = object_feats_att.reshape(bsz, max_frames, fsize)     #bs  26   1000
        ...
        # feat = torch.cat([object_feats_att, i3d_feats], dim=-1)    #bs  26  2024
        # pairwise_feat = self.self_attention(feat)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # self.self_attention = SelfAttention(opt.region_projected_size, opt.region_projected_size,
        #                                     opt.region_projected_size, opt.dropout, True)
    def _init_lstm_state(self, d):
        batch_size = d.size(0)
    ...
    def decode(self, frame_feats, i3d_feats, object_feats, previous_cells, att_lstm_h, lang_lstm_h, lang_lstm_c):
        ...
        # object_feats = self.self_attention(object_feats)
