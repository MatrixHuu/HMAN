...
class BeamSearch:
    ...
    def search(
        ...
        for timestep in range(self.max_steps - 1):
            ...
            for key, state_tensor in state.items():
                ...
                state[key] = (
                    ...
                    .gather(1, expanded_backpointer.to(torch.int64))
                    .reshape(batch_size * self.beam_size, *last_dims)
                )
        ...
        for timestep in range(len(predictions) - 2, 0, -1):
            # shape: (batch_size, beam_size, 1)
            cur_preds = predictions[timestep].gather(1, cur_backpointers.to(torch.int64)).unsqueeze(2)

            reconstructed_predictions.append(cur_preds)

            # shape: (batch_size, beam_size)
            cur_backpointers = backpointers[timestep - 1].gather(1, cur_backpointers.to(torch.int64))

        # shape: (batch_size, beam_size, 1)
        final_preds = predictions[0].gather(1, cur_backpointers.to(torch.int64)).unsqueeze(2)
