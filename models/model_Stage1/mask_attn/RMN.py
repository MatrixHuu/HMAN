...
class SoftAttention(nn.Module):
    def __init__(self, feat_size, hidden_size, att_size):
        ...
        self.wq = nn.Linear(hidden_size, att_size)
        self.wk = nn.Linear(feat_size, att_size)
        self.w = nn.Linear(att_size, 1, bias=False)
        self.b = nn.Parameter(torch.ones(self.hidden_size), requires_grad=True)


    def forward(self, keys, query, masks=None):
        Wh = self.wq(query)
        Uv = self.wk(keys)
        ...
        weighted_feats = keys * weights.expand_as(keys)
        attn_feats = weighted_feats.sum(dim=1)
        return attn_feats, weights

class PositionalEncoding_old(nn.Module):
    "Implement the PE function."
    def __init__(self, d_model, get_feats=False, dropout=0.2, max_len=72):
...
class SelfAttention(nn.Module):
    def __init__(self, input_size, attention_size, output_size, dropout=0.2, get_pe=False, get_feats=False):
        ...
        self.get_pe = get_pe
        self.get_feats = get_feats
        self.pe = PositionalEncoding_old(attention_size, self.get_feats)
...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        self.SelAtten = SelfAttention(opt.region_feature_size, opt.region_feature_size, opt.region_feature_size,
                                      opt.dropout, True)
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats):
        ...
        # region_feats self attention and get mask_index
        region_sel_feats, attn = self.SelAtten(region_feats)
        mask_index = self.mask(attn)
        ...
        # obj_outputs = self.obj_linear(object_feats)

        return frame_feats, i3d_feats, object_feats, mask_index


    def mask(self, attn):
        num_boxes = attn.size(-1)
        A = attn.matmul(attn.transpose(-1, -2))
        A_mask = torch.eye(num_boxes, num_boxes).cuda().type(torch.bool)
        ...
        indices = indices[indices[:, 2] < indices[:, 3]]  # Leave only the upper triangle to prevent duplication
        ...
        A_sum_i = A_sum[indices_b, indices_f, indices_i]
        A_sum_j = A_sum[indices_b, indices_f, indices_j]
        ...
        redundancy_masks[indices_b, indices_f, indices_i_or_j] = 1  # Mask phrases that are more redundant
...
class reasoning(nn.Module):
     def __init__(self, opt):
            ...
            self.spatial_attn = SoftAttention(opt.feature_size, opt.hidden_size, opt.hidden_size)
            ...
            self.semantic_attention = SemanticAttention(
                ...
                key_size=self.embedding_size + 3*self.vis_feat_size,
                bottleneck_size=self.sem_attn_hidden_size)

            self.cnnembedded = nn.Linear(opt.hidden_size * 4, opt.hidden_size * 2)

     def forward(self, frame_feats, i3d_feats, object_feats, phr_feats, last_hidden, phr_masks, mask):
            """
            ...
            semantic_frame_feats, _, _ = self.semantic_alignment(
                phr_feats=phr_feats,
            ...
            semantic_group_feats = torch.cat([semantic_obj_feats, semantic_frame_feats, semantic_i3d_feats, phr_feats], dim=2)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # attention lstm
        visual_feat_size = opt.hidden_size * 4 + opt.object_projected_size
        ...
        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.word_size*2+opt.feature_size*3+opt.hidden_size, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.word_size*2+opt.feature_size*3+2*opt.hidden_size, self.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, mask, teacher_forcing_ratio=1.0):
        ...
        att_lstm_h, att_lstm_c = self.get_rnn_init_hidden(batch_size, 512)
        ...
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                att_lstm_h, att_lstm_c = self.att_lstm(torch.cat([lang_lstm_h, global_feat, embedded], dim=1),
                ...
                output, lang_lstm_h, lang_lstm_c = self.decode(frame_feats, i3d_feats, object_feats, att_lstm_h,  lang_lstm_h, lang_lstm_c,
                                                               embedded, phr_feats, phr_masks, mask)
            ...
            outputs = self.beam_search(batch_size, self.vocab_size, frame_feats, i3d_feats, object_feats, global_feat,
                                       self.beam_size, mask)
        return outputs

    def decode(self, frame_feats, i3d_feats, object_feats, att_lstm_h, lang_lstm_h, lang_lstm_c, embedded, phr_feats, phr_masks, mask):

        feats = self.reasoning(frame_feats, i3d_feats, object_feats, phr_feats, att_lstm_h, phr_masks, mask)

        # language lstm decoder
        decoder_input = torch.cat([feats, embedded, att_lstm_h], dim=1)
    ...
    def beam_search(self, batch_size, vocab_size, frame_feats, i3d_feats, object_feats, global_feat, width, mask):
            ...
            for t in range(0, self.max_words):
                ...
                for i, (input, hidden, cum_prob) in enumerate(zip(input_list, hidden_list, cum_prob_list)):
                    ...
                    if t == 0:
                        ...
                        caption_lens = torch.cuda.LongTensor([[idx.item() for idx in caption].index(EOS_idx) if EOS_idx in [idx.item() for idx in caption] else t for caption in caption_list])
                        src_pos = torch.arange(1, t+1).repeat(batch_size, 1).cuda()
                        src_pos[src_pos > caption_lens[:, None]] = 0
                    phr_feats, phr_attns = self.phr_encoder(embedded_list, src_pos, return_attns=True)
                    phr_attns = phr_attns[0]
                    ...
                    att_lstm_h, att_lstm_c = self.att_lstm(torch.cat([hidden[0], global_feat, embedded], dim=1),
                                                           (att_lstm_h, att_lstm_c))
                    att_lstm_h = self.att_lstm_drop(att_lstm_h)

                    output, lang_lstm_h, lang_lstm_c = self.decode(frame_feats, i3d_feats, object_feats, att_lstm_h,
                                                                   hidden[0], hidden[1],
                                                                   embedded, phr_feats, phr_masks, mask)
...
class CapModel(nn.Module):
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats, captions, teacher_forcing_ratio=1.0):
        frame_feats, i3d_feats, object_feats, mask_index = self.encoder(cnn_feats, region_feats, spatial_feats)
        outputs = self.decoder(frame_feats, i3d_feats, object_feats, captions, mask_index, teacher_forcing_ratio)
