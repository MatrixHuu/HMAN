...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.word_size*2+opt.feature_size*3, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.word_size*2+opt.feature_size*3+opt.hidden_size, self.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, mask, teacher_forcing_ratio=1.0):
        ...
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                output, lang_lstm_h, lang_lstm_c = self.decode(frame_feats, i3d_feats, object_feats, lang_lstm_h, lang_lstm_c,
    ...
    def decode(self, frame_feats, i3d_feats, object_feats, lang_lstm_h, lang_lstm_c, embedded, phr_feats, phr_masks, mask):

        feats = self.reasoning(frame_feats, i3d_feats, object_feats, phr_feats, lang_lstm_h, phr_masks, mask)
    ...
    def beam_search(self, batch_size, vocab_size, frame_feats, i3d_feats, object_feats, width, mask):
