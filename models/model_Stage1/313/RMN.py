...
class SelfAttention(nn.Module):
    def __init__(self, input_size, attention_size, output_size, dropout=0.2, get_pe=False):
        ...
        self.get_pe = get_pe
        self.pe = PositionalEncoding_old(2*attention_size)
...
class SoftAttention(nn.Module):
    ...
    def forward(self, keys, query, masks=None):
        ...
        energies = self.w(torch.tanh(Wh + Uv))
...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        self.SelAtten = SelfAttention(opt.region_feature_size, opt.hidden_size*2, opt.hidden_size*2, opt.dropout, True)
