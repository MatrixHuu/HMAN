...
class global_reasoning(nn.Module):
     def __init__(self, opt):
            super(global_reasoning, self).__init__()
     ...
     def forward(self, frame_feats, i3d_feats,  phr_feats, last_hidden, phr_masks):
            """
...
class local_reasoning(nn.Module):
    def __init__(self, opt):
        super(local_reasoning, self).__init__()
    ...
    def forward(self, object_feats, phr_feats, last_hidden, phr_masks):
        """
        :param frame_feats: (batch_size, max_frames, 2*hidden_size)
        :param object_feats: (batch_size, max_frames, num_boxes, object_projected_size)
        :param phr_feats: (batch_size, word_size)
        ...
        semantic_group_feats = torch.cat([semantic_obj_feats, phr_feats],
                                         dim=2)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        self.lang_lstm1 = nn.LSTMCell(opt.word_size*2+opt.feature_size*3+opt.hidden_size, opt.hidden_size)
        self.lstm_drop1 = nn.Dropout(p=opt.dropout)
        ...
        self.global_reasoning = global_reasoning(opt)
        self.local_reasoning = local_reasoning(opt)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        lang_lstm1_h, lang_lstm1_c  = self.get_rnn_init_hidden(batch_size, 512)
        ...
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                output, lang_lstm_h, lang_lstm_c, lang_lstm1_h, lang_lstm1_c = self.decode(frame_feats, i3d_feats, object_feats, lang_lstm_h, lang_lstm_c,
                                                               lang_lstm1_h, lang_lstm1_c, embedded, phr_feats, phr_masks)
    ...
    def decode(self, frame_feats, i3d_feats, object_feats, lang_lstm_h, lang_lstm_c, lang_lstm1_h, lang_lstm1_c, embedded, phr_feats, phr_masks):

        global_feats = self.global_reasoning(frame_feats, i3d_feats, phr_feats, lang_lstm_h, phr_masks)
        local_feats = self.local_reasoning(object_feats, phr_feats, lang_lstm_h, phr_masks)

        # language lstm decoder
        decoder_input = torch.cat([global_feats, embedded], dim=1)
        ...
        decoder_input1 = torch.cat([local_feats, lstm_h, embedded], dim=1)
        lstm_h_1, lstm_c_1 = self.lang_lstm1(decoder_input1, (lang_lstm1_h, lang_lstm1_c))
        lstm_h_1 = self.lstm_drop1(lstm_h)
        decoder_output = torch.tanh(self.out_fc(torch.cat([lstm_h_1, decoder_input1], dim=1)))
        word_logits = self.word_restore(decoder_output)  # b*v
        word_logits = torch.log_softmax(word_logits, dim=1)

        return word_logits, lstm_h, lstm_c, lstm_h_1, lstm_c_1
    ...
    def beam_search(self, batch_size, vocab_size, frame_feats, i3d_feats, object_feats, width):
            ...
            for t in range(0, self.max_words):
                ...
                for i, (input, hidden, cum_prob) in enumerate(zip(input_list, hidden_list, cum_prob_list)):
                    ...
                    output, lang_lstm1_h, lang_lstm1_c, lang_lstm_h, lang_lstm_c, = self.decode(frame_feats, i3d_feats, object_feats,
                                                                    lang_lstm1_h, lang_lstm1_c,
                                                                    hidden[0], hidden[1],
