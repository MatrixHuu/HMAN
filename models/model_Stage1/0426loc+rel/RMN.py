...
class relate(nn.Module):
    ...
    def forward(self, i3d, object_feats, phr_feats, last_hidden, phr_masks):
        """
        :param frame_feats: (batch_size, max_frames, 2*hidden_size)
        :param object_feats: (batch_size, max_frames, num_boxes, object_projected_size)
        :param phr_feats: (batch_size, word_size)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.word_size*2+opt.feature_size*2, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.word_size*2+opt.feature_size*2+opt.hidden_size, self.hidden_size)
        ...
        self.locate = locate(opt)
        self.relate = relate(opt)
        self.embed = nn.Linear(4*opt.feature_size+2*opt.word_size, 2*opt.feature_size+opt.word_size)
    ...
    def forward(self, frame, i3d, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                output, lang_lstm_h, lang_lstm_c = self.decode(frame, i3d, object_feats, lang_lstm_h, lang_lstm_c,
            ...
            outputs = self.beam_search(batch_size, self.vocab_size, frame, i3d, object_feats,
                                       self.beam_size)
        return outputs

    def decode(self, frame, i3d, object_feats, lang_lstm_h, lang_lstm_c, embedded, phr_feats, phr_masks):

        frame_feats = self.locate(frame, object_feats, phr_feats, lang_lstm_h, phr_masks)
        motion_feats = self.relate(i3d, object_feats, phr_feats, lang_lstm_h, phr_masks)
        feats = self.embed(torch.cat([frame_feats, motion_feats], dim=-1))
    ...
    def beam_search(self, batch_size, vocab_size, frame, i3d, object_feats, width):
            ...
            for t in range(0, self.max_words):
                ...
                for i, (input, hidden, cum_prob) in enumerate(zip(input_list, hidden_list, cum_prob_list)):
                    ...
                    output, lang_lstm_h, lang_lstm_c = self.decode(frame, i3d, object_feats,
                                                                   hidden[0], hidden[1],
                                                                   embedded, phr_feats, phr_masks)
...
class CapModel(nn.Module):
    ...
    def forward(self, cnn_feats, region_feats, spatial_feats, captions, teacher_forcing_ratio=1.0):
        frame, i3d, object_feats = self.encoder(cnn_feats, region_feats, spatial_feats)
        outputs = self.decoder(frame, i3d, object_feats, captions, teacher_forcing_ratio)
        return outputs