...
class SelfAttention(nn.Module):
    def __init__(self, input_size, attention_size, output_size, dropout=0.2, get_pe=False):
        ...
        self.Ws = nn.Sequential(
            nn.ReLU(),
            nn.Linear(in_features=26, out_features=26, bias=True)
        )
    ...
    def forward(self, x, att_mask=None):
        ...
        logits = self.Ws(torch.matmul(K, Q))
...
class LOCATE(nn.Module):
     def __init__(self, opt):
        ...
        self.semantic_alignment = SemanticAlignment(
            ...
            feat_size=self.vis_feat_size*2,  # b,26,36,1024  or  b,26,1024
            bottleneck_size=self.sem_align_hidden_size)


        self.semantic_attention = SemanticAttention(
            query_size=self.hidden_size,
            key_size=self.embedding_size + self.vis_feat_size*2,
            bottleneck_size=self.sem_attn_hidden_size)



     def forward(self, frame_feats, object_feats, phr_feats, last_hidden, phr_masks):
            """
            ...
            con_feat = torch.cat([object_feats_att, frame_feats], dim=-1)
            ...
            semantic_group_feats = torch.cat([semantic_frame_feats, phr_feats],
                                             dim=2)
...
class RELATE(nn.Module):
    def __init__(self, opt):
        ...
        self.semantic_alignment = SemanticAlignment(
            query_size=self.embedding_size,  # b,step,300
            feat_size=self.vis_feat_size*4,  # b,26,36,1024  or  b,26,1024
            bottleneck_size=self.sem_align_hidden_size)


        self.semantic_attention = SemanticAttention(
            query_size=self.hidden_size,
            key_size=self.embedding_size + self.vis_feat_size*4,
            bottleneck_size=self.sem_attn_hidden_size)



    def forward(self, i3d_feats, object_feats, phr_feats, last_hidden, phr_masks):
        ...
        con_feat = torch.cat([object_feats_att, i3d_feats], dim=-1)
        feat1 = con_feat.repeat(1, max_frames, 1)
        feat2 = con_feat.repeat(1, 1, max_frames).reshape(bsz, max_frames * max_frames, -1)
        pairwise_feat = torch.cat([feat1, feat2], dim=-1)

        semantic_i3d_feats, _, _ = self.semantic_alignment(
            phr_feats=phr_feats,
            vis_feats=pairwise_feat)

        semantic_group_feats = torch.cat([semantic_i3d_feats, phr_feats],
                                         dim=2)
...
class FUNC(nn.Module):
    def __init__(self, opt):
        ...
        self.cell_attn = SemanticAttention(opt.hidden_size, opt.hidden_size, opt.hidden_size)
    ...
    def forward(self, cells, hidden_state):
        ...
        :return: func_feat
        '''
        func_feat, _, _ = self.cell_attn(hidden_state, cells, cells)
        return func_feat
    ...
    def __init__(self, opt):
        ...
        if opt.use_loc and opt.use_rel and opt.use_func:
            if opt.attention == 'soft':
                self.module_attn = SemanticAttention(opt.hidden_size, opt.hidden_size, opt.hidden_size)
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        # language lstm
        self.lang_lstm = nn.LSTMCell(opt.word_size + opt.hidden_size, opt.hidden_size)
        self.lstm_drop = nn.Dropout(p=opt.dropout)

        # final output layer
        self.out_fc = nn.Linear(opt.word_size + opt.hidden_size*2, self.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                output, module_weight, lang_lstm_h, lang_lstm_c = self.decode(frame_feats, i3d_feats, object_feats, previous_cells, lang_lstm_h, lang_lstm_c,
    ...
    def decode(self, frame_feats, i3d_feats, object_feats, previous_cells, lang_lstm_h, lang_lstm_c, embedded, phr_feats, phr_masks):
        ...
        loc_feats = self.loc(frame_feats, object_feats, phr_feats, lang_lstm_h, phr_masks) if self.use_loc else None
        rel_feats = self.rel(i3d_feats, object_feats, phr_feats, lang_lstm_h, phr_masks) if self.use_rel else None
        func_feats = self.func(previous_cells, lang_lstm_h) if self.use_func else None
        feats, module_weight = self.module_selection(loc_feats, rel_feats, func_feats, lang_lstm_c)
    ...
    def beam_search(self, batch_size, vocab_size, frame_feats, i3d_feats, object_feats, width):
            ...
            previous_cells = []
            previous_cells.append(hidden[1])
            ...
            for t in range(0, self.max_words):
                ...
                for i, (input, hidden, cum_prob) in enumerate(zip(input_list, hidden_list, cum_prob_list)):
                    ...
                    output, module_weight, lang_lstm_h, lang_lstm_c = self.decode(frame_feats, i3d_feats, object_feats, previous_cells,
                                                                   hidden[0], hidden[1],
                                                                   embedded, phr_feats, phr_masks)
