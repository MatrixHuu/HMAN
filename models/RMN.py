...
from HMN.model.entity_level import EntityLevelEncoder
from HMN.model.transformer import Transformer
# ------------------------------------------------------
...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        self.i3d_feature_embed = nn.Linear(2*opt.m_feature_size, opt.frame_projected_size)
    ...
    def forward(self, frame_feats, i3d_feats):
        '''
...
class Decoder(nn.Module):
    def __init__(self, opt, vocab):
        ...
        self.att_lstm = nn.Linear(att_insize, opt.hidden_size)
    ...
    def forward(self, frame_feats, i3d_feats, object_feats, captions, teacher_forcing_ratio=1.0):
        ...
        caption_EOS_table = captions == 0
        caption_end_table = caption_EOS_table
        ...
        output = Variable(torch.cuda.LongTensor(1, batch_size).fill_(1))
        if not infer or self.beam_size == 1:
            for t in range(0, self.max_words):
                ...
                if t == 0:
                    ...
                    caption_lens += (output.long().squeeze() != 0).long()
    ...
    def beam_search(self, batch_size, vocab_size, frame_feats, i3d_feats, object_feats, width):
            ...
            input_list = [torch.cuda.LongTensor(1, batch_size).fill_(1)]
            hidden_list = [hidden]
...
class CapModel(nn.Module):
    ...
    def forward(self, frame_feats, motion_feats, object_feats, objects_mask, captions, teacher_forcing_ratio=1.0):
        objects_feats, objects_semantics = self.entity_level(frame_feats, motion_feats, object_feats, objects_mask)
        frame_feats, i3d_feats = self.encoder(frame_feats, motion_feats)
        outputs = self.decoder(frame_feats, i3d_feats, objects_feats, captions, teacher_forcing_ratio)
        return objects_semantics, outputs
